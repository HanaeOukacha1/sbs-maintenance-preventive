...
'''
    ...
    # 6. Remove the IIFE wrappers and the function definitions
    # Match the start of the IIFE down to the end of PcFields definition
    # Then just replace the very last `})()}` with `}`
    
    start_iife = r'\{\(\(\) => \{\s*const F =.*?const PcFields = [^\n]+\n\s*'
    content = re.sub(start_iife, '{', content, flags=re.DOTALL)
    
    # We still have `})()}` at the end of the IIFE block (around line 500)
    # Since there is only one `})()}` at the end of the block, we can replace it with `}`
