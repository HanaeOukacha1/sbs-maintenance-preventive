...
with engine.connect() as conn:
    ...
    # 2. Affecter uniquement AOH
    sites = conn.execute(text("SELECT id, nom, ville FROM sites WHERE checklist_type = 'AOH'")).fetchall()
    ...
    for site_id, nom, ville in sites:
        ...
        ), {'titre': f"MP AOH {ville or nom}", 'sid': site_id, 'tid': tech_id})
    conn.commit()
    print(f"AOH: {len(sites)} mission(s) créée(s)")
