...
with engine.connect() as conn:
    # Cleanup previous CNDH sites/missions/equipments
    sites = conn.execute(text("SELECT id FROM sites WHERE checklist_type LIKE 'CNDH%'")).fetchall()
    for (site_id,) in sites:
        conn.execute(text(f"DELETE FROM equipements WHERE site_id = {site_id}"))
        conn.execute(text(f"DELETE FROM interventions WHERE mission_id IN (SELECT id FROM missions WHERE site_id = {site_id})"))
        conn.execute(text(f"DELETE FROM missions WHERE site_id = {site_id}"))
        conn.execute(text(f"DELETE FROM sites WHERE id = {site_id}"))
    conn.commit()

    # Get marche_id
    marche_id = 11
    marche_db = conn.execute(text("SELECT id FROM marches WHERE nom LIKE '%CNDH%' LIMIT 1")).fetchone()
    if marche_db: marche_id = marche_db[0]
        ...
        try:
            ...
            for sheet in xl.sheet_names:
                ...
                if header_idx is not None:
                    valid_sheets.append((sheet, df, header_idx))
                    
            if not valid_sheets:
                continue

            # Determine feuilles to save in site
            feuilles_list = [s[0] for s in valid_sheets] if len(valid_sheets) > 1 and valid_sheets[0][0].upper() != 'A' else []
            feuilles_json = json.dumps(feuilles_list) if feuilles_list else None
            ...
            for sheet, df, header_idx in valid_sheets:
                ...
                for _, row in df.iloc[header_idx+1:].iterrows():
                    ...
                    # Correction des inversions de colonnes (ex: Oujda a mis le site dans AFFECTATION)
                    ...
                    sous_site = sheet if feuilles_list else None
                ...
                if eqs_to_insert:
                    for eq in eqs_to_insert:
                        conn.execute(text(
                            "INSERT INTO equipements (site_id, sous_site, direction, bureau, utilisateur_nom, famille, marque, modele, numero_serie, type_equipement, is_active) "
                            "VALUES (:site_id, :sous_site, :direction, :bureau, :utilisateur_nom, :famille, :marque, :modele, :numero_serie, :type_equipement, 1)"
                        ), eq)
                    conn.commit()
    ...
    print(f"Done. Processed {processed_sites} unique sites. Inserted {total_equipments} new equipments.")
