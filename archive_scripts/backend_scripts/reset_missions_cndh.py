...
with engine.connect() as conn:
    # 1. Supprimer toutes les missions
    ...
    # 2. Affecter uniquement CNDH
    sites = conn.execute(text("SELECT id, nom, ville, checklist_type FROM sites WHERE checklist_type LIKE 'CNDH%'")).fetchall()
    
    for site_id, nom, ville, ctype in sites:
        conn.execute(text(
            "INSERT INTO missions (titre, site_id, technicien_id, statut, date_planifiee) "
            "VALUES (:titre, :sid, :tid, 'PLANIFIEE', CURDATE())"
        ), {'titre': f"MP {nom}", 'sid': site_id, 'tid': tech_id})
    conn.commit()
    print(f"CNDH: {len(sites)} mission(s) créée(s)")
