...
with engine.connect() as conn:
    ...
    # 2. Affecter uniquement ANP
    sites = conn.execute(text("SELECT id, nom, ville FROM sites WHERE checklist_type = 'ANP'")).fetchall()
    ...
    for site_id, nom, ville in sites:
        ...
        ), {'titre': f"MP ANP {ville or nom}", 'sid': site_id, 'tid': tech_id})
    conn.commit()
    print(f"ANP: {len(sites)} mission(s) créée(s)")
