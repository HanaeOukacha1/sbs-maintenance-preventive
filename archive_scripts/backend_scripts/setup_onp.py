...
with engine.connect() as conn:
    # 1. Get ONP marche
    marche_id = 9 # Let's try to find it dynamically first
    marche = conn.execute(text("SELECT id FROM marches WHERE nom LIKE '%ONP%' LIMIT 1")).fetchone()
    if marche:
        marche_id = marche[0]
    else:
        # Check existing ONP sites
        site_m = conn.execute(text("SELECT marche_id FROM sites WHERE checklist_type = 'ONP' LIMIT 1")).fetchone()
        if site_m:
            marche_id = site_m[0]

    print(f"ONP marche_id: {marche_id}")
    
    tech_id = 4 # hanae
    
    # 2. Extract sites and equipments from Excel
    # Headers are on row 5: N | SITE | DESIGNATION | MARQUE | MODELE | N DE SERIE
    # N°=0, SITE=1, DESIGNATION=2, MARQUE=3, MODELE=4, SERIE=5
    ...
    # 3. Create sites/missions/equipments in DB
    for site_name, equipments in sites_dict.items():
        ...
        if site_db:
            ...
            print(f"Created site {site_id} for {nom_complet}")
        ...
        if mission_db:
            ...
            print(f"Created mission {mission_id} for {nom_complet}")
        ...
        if count_eq == 0:
            ...
            conn.commit()
            print(f"Inserted {len(equipments)} equipments for {nom_complet}")
        
    print("Done handling ONP DB.")
