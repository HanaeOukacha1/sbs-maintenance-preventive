...
'''
    ...
    # 5. Remove the function definitions ONLY, keeping the IIFE
    # Matches from the start of the IIFE down to the end of the PcFields definition
    start_iife = r'(\{\(\(\) => \{\s*)const F =.*?const PcFields = .*?</>\);\n(\s*)'
    content = re.sub(start_iife, r'\1\2', content, flags=re.DOTALL)
