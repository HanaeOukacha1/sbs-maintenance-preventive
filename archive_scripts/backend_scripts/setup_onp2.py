...
with engine.connect() as conn:
    ...
    for s in list(sites_dict.keys())[:3]:
        print(f"  {s}: {len(sites_dict[s])} items")
        
    # delete dummy SITE if created
    conn.execute(text("DELETE FROM sites WHERE nom = 'ONP SITE'"))
    conn.commit()
    ...
    print(f"Done. Processed {count_processed} sites with equipments inserted.")
