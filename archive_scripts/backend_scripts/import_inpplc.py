...
file_path = r'C:\Users\hanae\Desktop\Stage PFA 2026\MASTER DATA\INPPLC\Masters Data INPPLC.XLS'

print("Setting up INPPLC...")

# Fetch INPPLC marche_id
marche_id = conn.execute(text("SELECT id FROM marches WHERE nom LIKE '%INPPLC%' LIMIT 1")).fetchone()
if not marche_id:
    # Try finding it by keyword if INPPLC is not the exact name
    marche_id = conn.execute(text("SELECT id FROM marches WHERE nom LIKE '%Probité%' LIMIT 1")).fetchone()
    if not marche_id:
        print("Error: Marche INPPLC not found in DB.")
        exit(1)
marche_id = marche_id[0]

# Delete old INPPLC data if any
...
# Create Site INPPLC
site_nom = "INPPLC Rabat"
city_name = "RABAT"
default_checklist = 'INPPLC'
tech_id = 4 # hanae

# Parse Excel to get sheet names
xls = pd.ExcelFile(file_path, engine='xlrd')
feuilles_list = xls.sheet_names
...
), {'titre': "MP INPPLC Rabat", 'site_id': site_id, 'tech': tech_id})
conn.commit()

inserted = 0

for sheet_name in feuilles_list:
    df = pd.read_excel(file_path, sheet_name=sheet_name, header=8) # Row 8 is header (0-indexed)

    headers = [str(x).lower().strip() for x in df.columns]
    col_map = {}
    for idx, h in enumerate(headers):
        if 'famille' in h or 'article' in h: col_map['famille'] = idx
    ...
    if eqs_to_insert:
        conn.execute(
            text("""
                ...
                (site_id, sous_site, type_equipement, designation, marque, modele, numero_serie)
                VALUES 
                (:site_id, :sous_site, :type_equipement, :designation, :marque, :modele, :numero_serie)
            """),
            eqs_to_insert
...
print(f"Done. Imported {inserted} equipments for INPPLC with sub-sites.")
